...
const post_message_1 = __nccwpck_require__(252);
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield (0, post_message_1.postMessage)(inputs);
        }
        catch (error) {
...
function buildMessage(inputs) {
    const { status, scopes, version, userIds, repository, stage, commits, runUrl, sourceUrl, username } = inputs;
    const blocks = [];
    const headerText = status === 'success'
    ...
    if (version) {
        ...
        const historyText = commits.map(c => {
            ...
            const message = lines
                ...
                .filter(l => l !== '')
                .join(', ')
                .replace(/([a-zA-Z/_-]+)#(\d+)/, `<https://github.com/$1/issues/$2|#$2>`);
            return `*${type}*: ${message}`;
        });
        const history = {
            type: 'section',
...
function postMessage(inputs) {
    return __awaiter(this, void 0, void 0, function* () {
        const web = new web_api_1.WebClient(inputs.slackToken);
        ...
        const result = yield web.chat.postMessage(message);
        if (result.error)
